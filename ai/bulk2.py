...
def tmp_path(suffix):
    """Create and return a unique temp file path with given suffix (closed)."""
...
def convert_stp_to_image(stp_path, output_dir):
    """
    Convert one STEP file to a shaded PNG and return the output path, or None on failure.
    """
    try:
        # STEP ➔ STL
        ...
        # Render STL with normals, shading
    ...
    for rel_path, abs_path, ext in file_list:
        try:
            if ext == '.pdf':
                ...
                png_path = convert_stp_to_image(abs_path, OUTPUT_IMAGES_DIR)
                if png_path:
                    obj = client.files.upload(file=png_path, config=dict(mime_type='image/png'))
                    uploaded[rel_path] = obj
                    print(f"STP → PNG: {rel_path}  ➜  {png_path}")
                else:
